...
static glm::mat4 translationMatrix(float tx, float ty, float tz) {
  glm::mat4 m(0.0f);
...
static glm::mat4 scalingMatrix(float sx, float sy, float sz) {
  glm::mat4 m(0.0f);
  m[0][0] = sx; m[1][1] = sy; m[2][2] = sz; m[3][3] = 1.0f;
  return m;
}
